...
- **Multilateration (TDOA)**: when 3+ sensors with known positions and timestamps hear the same `(from, packet_id)` within the dedup window, the fusion runs a hyperbolic-TDOA solver (Levenberg-Marquardt, multi-start to break the 3-station hyperbolic ambiguity) and emits a `GEOLOCATED` event with the estimated emitter lat/lon and 1-sigma uncertainty in meters. The Live tab map renders these as magenta diamond markers with a confidence-radius circle.
- **Clock-sync (no-GPSDO TDOA)**: stations without GPSDO/PPS can participate in volunteer-grade TDOA via `--calibration-node=...` declarations. Fusion synthesizes a per-network clock by observing operator-anchored transmitters at known positions; pair offsets are estimated with a robust median + MAD per station pair. The new `timestamp_class=sync` label appears on `GEOLOCATED` events once the contributing pairs converge. See [docs/clock-sync.md](../docs/clock-sync.md) for anchor declaration syntax, placement guidance (anchors must be ≥ 30 m from sniffer stations to avoid near-field saturation), tuning knobs, and the "what clock-sync is NOT" notes.
- **Timestamp precision tiers** (best to worst, surfaced as `timestamp_class` on `GEOLOCATED`):
  - `sample` — sample-counter-derived TOA from a GPSDO/PPS-equipped sniffer (planned, `tdoa-sample-epoch`)
  - `sync` — `software_lock` corrected by converged clock-sync pair offsets
  - `software_lock` — `preamble_lock_t_ns` from the sniffer (`CLOCK_REALTIME` stamped at preamble-detect); no network correction yet
  - `frame` — legacy `station_t_ns` (stamped at frame-emit; includes whole-frame demod latency). The fusion solver weights each observation by `1/station_t_acc_ns` so well-disciplined stations dominate, AND respects the timestamp class so a converged-sync solve labels itself accordingly.
